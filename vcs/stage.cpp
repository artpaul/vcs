...
auto StageArea::Directory::FromTree(const Tree& tree) -> std::unique_ptr<Directory> {
    ...
    for (const auto& e : tree.Entries()) {
        dir->entries_.emplace(e.Name(), Entry{.id = e.Id(), .type = e.Type(), .size = e.Size()});
    }
            ...
            if (ei->type == PathType::Directory) {
                ...
                for (const auto& e : tree.Entries()) {
                    entries.emplace_back(
                        e.Name(), PathEntry{.id = e.Id(), .type = e.Type(), .size = e.Size()}
                    );
                }
            }
        }
...
HashId StageArea::SaveTree(Datastore* odb) const {
    if (stage_root_) {
        return SaveTreeImpl(stage_root_.get(), odb);
    } else {
        return tree_id_;
    }
}
...
HashId StageArea::SaveTreeImpl(const Directory* root, Datastore* odb) const {
    TreeBuilder builder;
