...
void pipe_thread(std::string& input, PipeHandle output, bool bautoclose) {
  std::thread thread([input(move(input)), output, bautoclose]() {
    ...
    while (pos < input.size()) {
      ssize_t transfered =
        pipe_write(output, input.c_str() + pos, input.size() - pos);
      if (transfered <= 0)
        break;
      pos += transfered;
    }
...
bool Popen::poll() {
  ...
  if (result == WAIT_TIMEOUT) {
    ...
    DWORD error = GetLastError();
    throw OSError("WAIT_FAILED error:" + std::to_string(error) + ":" +
                  lastErrorString());
  }
  ...
  if (ret == 0) {
    DWORD error = GetLastError();
    throw OSError("GetExitCodeProcess failed: " + std::to_string(error) + ":" +
                  lastErrorString());
  }
